    ...
    <header className="sticky top-0 z-30 border-b border-black/5 bg-[linear-gradient(180deg,rgba(253,251,247,0.92),rgba(253,251,247,0.8))] px-5 py-5 backdrop-blur md:px-8 xl:grid xl:grid-cols-[auto_1fr_auto] xl:items-center xl:gap-6 2xl:px-[60px]">
      <div className="flex items-center justify-between gap-4 xl:contents">
        <Link href="/" aria-label={`${siteConfig.name}, voltar para a página inicial`} className="min-w-0">
          <span className="block font-display text-[clamp(1.62rem,2.2vw,2.12rem)] leading-none tracking-[-0.04em] text-ink">
            {siteConfig.name}
          </span>
          <span className="mt-1 block text-[0.72rem] uppercase tracking-[0.18em] text-ink-faint">
      ...
      <nav className="hidden flex-wrap items-center justify-center gap-x-6 gap-y-3 text-[0.93rem] text-ink/80 xl:flex" aria-label="Principal">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
      ...
      >
        <nav className="grid gap-3 pb-4 text-sm text-ink/80" aria-label="Principal no celular">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
